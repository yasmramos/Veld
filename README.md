...
### 4. Use the Generated Veld Class

Veld generates a unified `Veld` class with static methods for zero-overhead access:
...
public class Main {
    public static void main(String[] args) {
        // Direct static access (recommended - zero overhead)
        UserService userService = Veld.userService();
        User user = userService.findUser(1L);
        
        // Dynamic lookup (when type is known at runtime)
        UserService service = Veld.get(UserService.class);
    }
}
```

> **Note**: The generated `Veld` class uses `static final` fields initialized in a static block, providing thread-safe, zero-cost access to all singletons.
...
2. **Analysis**: Builds a dependency graph, resolves dependencies, and validates for cycles
3. **Generation**: Creates a unified `Veld` class with all singletons as `static final` fields
4. **Weaving**: `veld-weaver` adds synthetic setters for private field injection
5. **Runtime**: Direct static field access - no container, no reflection, no lookups
6. **AOP**: ProxyFactory generates bytecode proxies that intercept method calls

### Architecture: Unified Veld Class

Unlike traditional DI containers that use HashMaps and runtime lookups, Veld generates a single unified class where:

- **All singletons are `static final` fields** - JVM guarantees thread-safe initialization
- **Direct getter methods** - `Veld.userService()` compiles to a single field read
- **No container overhead** - No HashMap, no dynamic dispatch, no boxing
- **JIT-friendly code** - The JVM can inline everything
...
Veld generates a unified class:

```java
public final class Veld {
    // All singletons as static final fields (thread-safe, zero-cost)
    private static final LogService _logService;
    private static final UserService _userService;
    
    // Static initializer - JVM guarantees thread-safe execution
    static {
        _logService = new LogService();
        _userService = new UserService();
        _userService.__di_set_logService(_logService);
    }
    
    private Veld() {} // Cannot instantiate
    
    // Direct getters (JIT inlines to field read)
    ...
    // Dynamic lookup (for runtime type resolution)
    @SuppressWarnings("unchecked")
    public static <T> T get(Class<T> type) {
        if (type == LogService.class) return (T) _logService;
        if (type == UserService.class) return (T) _userService;
        return null;
    }
}
```

**For private field injection**, `veld-weaver` generates synthetic setters:
