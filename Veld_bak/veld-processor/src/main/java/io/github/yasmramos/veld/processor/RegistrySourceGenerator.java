...
public final class RegistrySourceGenerator {
    ...
    private void addInstanceFields(TypeSpec.Builder classBuilder) {
        // factories array
        FieldSpec factoriesField = FieldSpec.builder(
    ...
    private void addStaticInitializer(TypeSpec.Builder classBuilder) {
        ...
        // Build the static initializer using CodeBlock to avoid indentation issues
        CodeBlock.Builder staticInitBuilder = CodeBlock.builder();
        // Use add() for multi-line content, not addStatement()
        staticInitBuilder.add(typeIndicesInit.toString().trim());
        staticInitBuilder.add("\n");
        staticInitBuilder.add(nameIndicesInit.toString().trim());
        staticInitBuilder.add("\n");

        // SCOPES array - build as a single statement
        StringBuilder scopesInit = new StringBuilder();
        scopesInit.append("SCOPES = new String[] {");
        for (int i = 0; i < components.size(); i++) {
            ComponentInfo comp = components.get(i);
            String scopeId = comp.getScopeId();
            if (i > 0) scopesInit.append(", ");
            scopesInit.append("\"").append(scopeId).append("\"");
        }
        scopesInit.append("}");
        staticInitBuilder.addStatement(scopesInit.toString());
        ...
        staticInitBuilder.addStatement(lazyFlagsInit.toString());
        ...
        for (Map.Entry<String, List<Integer>> entry : supertypeIndices.entrySet()) {
            ...
            staticInitBuilder.addStatement(supertypeIndicesInit.toString());
            supertypeIndicesInit.setLength(0);  // Clear for next iteration
        }
    ...
    private void addGetFactoriesForType(TypeSpec.Builder classBuilder) {
        // Match the interface signature exactly: <T> List<ComponentFactory<? extends T>> getFactoriesForType(Class<T> type)
        // The return type uses wildcards for covariance, but parameter type is Class<T> (not wildcards)
        TypeName factoryExtendsT = ParameterizedTypeName.get(ClassName.get(ComponentFactory.class),
                WildcardTypeName.subtypeOf(TypeVariableName.get("T")));
        TypeName listFactoryExtendsT = ParameterizedTypeName.get(ClassName.get(List.class), factoryExtendsT);
        ...
        MethodSpec getFactoriesForType = MethodSpec.methodBuilder("getFactoriesForType")
                ...
                .endControlFlow()
                .addStatement("return ($T) new $T<>(list)", listFactoryExtendsT, ArrayList.class)
                .build();
