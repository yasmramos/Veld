...
class LifecycleProcessorTest {
    ...
    // Helper to create MethodHandle from bean method (zero-reflection mode)
    private MethodHandle createMethodHandle(Object bean, String methodName) {
        try {
            return MethodHandles.lookup().findVirtual(
                bean.getClass(), methodName, MethodType.methodType(void.class)
            ).bindTo(bean);
        } catch (Exception e) {
            throw new RuntimeException("Failed to create MethodHandle: " + methodName, e);
        }
    }
        ...
        void shouldScanLifecycleCallbacksOnRegistration() {
            ...
            try {
                processor.registerPostInitialize(bean, "simpleBean",
                    createMethodHandle(bean, "postInit"), 0);
        ...
        void shouldInvokeOnStartCallbacks() {
            ...
            try {
                processor.registerOnStart(bean, "simpleBean",
                    createMethodHandle(bean, "onStart"), 0);
            } catch (NoSuchMethodException e) {
                fail("Method not found: " + e.getMessage());
            }
        ...
        void shouldInvokeOnStopCallbacks() {
            ...
            try {
                processor.registerOnStop(bean, "simpleBean",
                    createMethodHandle(bean, "onStop"), 0);
            } catch (NoSuchMethodException e) {
                fail("Method not found: " + e.getMessage());
            }
        ...
        void shouldInvokePostInitializeInOrder() {
            ...
            try {
                processor.registerPostInitialize(bean1, "bean1",
                        createMethodHandle(bean1, "first"), 1);
                processor.registerPostInitialize(bean2, "bean2",
                        createMethodHandle(bean2, "first"), 1);
            } catch (NoSuchMethodException e) {
                fail("Method not found: " + e.getMessage());
            }
